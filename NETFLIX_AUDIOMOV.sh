...
echo "
$0
arg1: must be an access path (cygwin/linux syntax) to a .wav files containing either a 5.1 PCM audio stem or a stereo PCM audio stream.
...
[ $WAV1_NbSTREAMS -gt 1 ] && usage "first audio file $WAVFILE1. Number of streams are greater than one. (only one audio stream is expected: it should either be 5.1 or Stereo)." 
...
# By default, let's state that there's only one valid mix at this stage.
TWOVALIDMIX="False"
# Let's valid if a second mix is given as arg2 and if this mix is valid against the first one.
if [[ $WAVFILE2 != "" ]]; then
  ...
  # if first wavefile is a 5.1 audio the second optional wav file needs to contain a stereo audio stream (vice versa). PCM
  # detect number of stream (it should be one as only the first audio stream will be used
...
if [[ $WAVFILE2 != "" ]] && [[ $TWOVALIDMIX == "True" ]]; then
  [ $WAV1_FORMAT = 6 ] && SNDMIX="$WAVFILE1_WINPATH" && LRMIX="$WAVFILE2_WINPATH"
  [ $WAV1_FORMAT = 2 ] && SNDMIX="$WAVFILE2_WINPATH" && LRMIX="$WAVFILE1_WINPATH"
  echo "
  5.1 mix is: $SNDMIX
  2.0 mix is: $LRMIX"
  # ffmpeg two audio mixes scenario
  ffmpeg -i "$SNDMIX" -i "$LRMIX" -map 0:a:0 -map 1:a:0 -c:a pcm_s24le "$WAVFILE1_WINPATH"_NFLX.mov
  else
  #ffmpeg one audio mix scenario
  ffmpeg -i "$WAVFILE1_WINPATH" -c:a pcm_s24le "$WAVFILE1_WINPATH".mov
